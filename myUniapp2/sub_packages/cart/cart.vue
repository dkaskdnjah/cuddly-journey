	...
	<view>
		...
		<u-overlay :show="show" @click="show = false">
			<view class="overlayContent bg-fff margin" @click.stop v-if="show">
				<view class="padding">
					<view class="flex">
						<image class="imgLogin margin-right-sm" src="http://lc-B0CeFVGF.cn-n1.lcfile.com/1qjq4zMqlMgRbEVrlKIvOij9NkpKQGMU/logo.png" mode="widthFix"></image>
						<view class="">
							{{editData.name}}
							<view class="margin-tb-xs">
								{{editData.french}}
		...
		<view class="flex fixed bg-fff justify-between">
			...
			<view @click="handleOrder" class="padding color-fff bg-yellow">
				立即结算
			</view>
		</view>
	</view>
		...
		onLoad() {
			...
			uni.showModal({
				...
				success({cancel}) {
					...
					uni.navigateTo({
						url: '/sub_packages/kitty-BasicLogin/login'
					})
				}
			})
		}
